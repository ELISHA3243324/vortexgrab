...
try {
    ...
    console.error('❌ yt-dlp not found or not working:', e.message);
}
...
app.post('/api/info', async (req, res) => {
    ...
    try {
        ...
        ytdlp.stderr.on('data', (data) => {
            ...
            console.log(`⚠️ yt-dlp stderr: ${data.toString()}`);
        });
        ...
        ytdlp.on('close', (code) => {
            ...
            if (code !== 0) {
                ...
                return res.status(400).json({ error: 'Could not fetch video info', details: errorOutput || 'Unknown error' });
            }
...
app.get('/api/test', (req, res) => {
    try {
        const version = execSync('yt-dlp --version').toString().trim();
        const ffmpeg = execSync('ffmpeg -version 2>&1 | head -1').toString().trim();
        res.json({ 
            status: 'ok', 
            ytdlp: version,
            ffmpeg: ffmpeg
